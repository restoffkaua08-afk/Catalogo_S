...
export function emailProviderStatus() {
  ...
  const hasSender = Boolean(String(process.env.AUTH_FROM_EMAIL || '').trim());
  ...
  let providers = [];
  if (preferred === 'mailjet') providers = ['mailjet', 'resend'];
  else if (preferred === 'resend') providers = ['resend', 'mailjet'];
  else providers = ['mailjet', 'resend'];

  providers = providers.filter((provider) =>
    provider === 'mailjet' ? mailjetReady : resendReady,
  );

  const missing = [];
  if (!hasSender) missing.push('AUTH_FROM_EMAIL');
  if (providers.length === 0) {
    if (preferred === 'mailjet') missing.push('MAILJET_API_KEY', 'MAILJET_SECRET_KEY');
    else if (preferred === 'resend') missing.push('RESEND_API_KEY');
    else missing.push('MAILJET_API_KEY+MAILJET_SECRET_KEY ou RESEND_API_KEY');
  }

  return {
    ok: hasSender && providers.length > 0,
    preferred,
    providers,
    missing,
  };
